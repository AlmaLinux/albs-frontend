<template>
  <q-dialog persistent position="top" v-model="opened" full-height style="max-width: 650px;">
          ...
          <q-card-section style="width: 650px; padding-left: 25px;">
            ...
            <div>
              <q-form @submit="onAddMacro" class="q-gutter-md">
                <div class="row no-wrap group">
                  ...
                  <q-input v-model="macroInput.value"
                          ...
                          style="margin-left: 20px; width: 262px;"></q-input>
                  <q-item-section side>
              ...
              <q-list v-if="Object.keys(mock_options.definitions).length" dense highlight no-border >
                <q-scroll-area :style="scrollStyle(Object.keys(mock_options.definitions).length)">
                  <q-item v-for="macro in mock_options.definitions" :key="macro.name" dense>
                    <q-item-section style="font-size: 11pt; letter-spacing: 1pt;">
            ...
            <q-select v-model="mock_options.with" label="With" use-chips hide-dropdown-icon input-debounce="0"
                      :rules="[val => mockValue(val) || 'Invalid With value']" class="q-pr-md"
            ...
            <q-select v-model="mock_options.without" label="Without" use-chips hide-dropdown-icon
                      :rules="[val => mockValue(val) || 'Invalid Without value']" class="q-pr-md"
                      new-value-mode="add-unique" use-input multiple clearable></q-select>
            <q-select v-model="mock_options.yum_exclude" label="Yum Exclude" use-chips hide-dropdown-icon
                      :rules="[val => yumExcludeName(val) || 'Invalid Yum Exclude value']" class="q-pr-md"
                      new-value-mode="add-unique" use-input multiple clearable></q-select>
            <q-select v-model="mock_options.module_enable" label="Enable Modules" use-chips hide-dropdown-icon
                      new-value-mode="add-unique" use-input multiple clearable class="q-pr-md">
            ...
            <q-input v-model="mock_options.target_arch" label="Target Arch" clearable autogrow hint="--target" class="q-pr-md">
              <q-tooltip anchor="bottom middle" self="top middle" :delay="1000">
    ...
    open (mock_options) {
      if (mock_options.definitions){
        ...
        for (const key in mock_options.definitions) {
          this.mock_options.definitions.push({ name: key, value: mock_options.definitions[key]})
        }
      } else {
        this.mock_options.definitions = []
      }
      mock_options.with ? this.mock_options.with = mock_options.with : this.mock_options.with = []
      mock_options.without ? this.mock_options.without = mock_options.without : this.mock_options.without = []
      mock_options.yum_exclude ? this.mock_options.yum_exclude = mock_options.yum_exclude : this.mock_options.yum_exclude = []
      mock_options.module_enable ? this.mock_options.module_enable = mock_options.module_enable : this.mock_options.module_enable = []
      mock_options.target_arch ? this.mock_options.target_arch = mock_options.target_arch : this.mock_options.target_arch = ''
      this.opened = true
    },
    close () {
      this.opened = false
