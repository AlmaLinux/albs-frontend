<template>
  <q-dialog v-model="opened" @hide="close">
    <q-card style="width: 100%">
      ...
      <q-card-section>
        <div class="text-h5">
          Edit Team Roles for user <b>{{ user.username }}</b>
        </div>
      </q-card-section>
...
<script>
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      close() {
        ;(this.teamOptions = []), (this.activeTeam = this.noTeam)
        this.teamModel = null
        this.opened = false
      },
      ...
      async loadData() {
        ...
        this.originalUserTeamRoles = JSON.parse(
          JSON.stringify(this.userTeamRoles)
        )
      ...
      patchRequest(action, updates) {
        return this.$api
          .patch(`/users/${this.user.id}/roles/${action}`, updates)
          .then((response) => {
            return response
          })
          .catch((error) => {
      ...
      teamSelectFilter(value, update) {
        update(() => {
          this.options = this.teamOptions.filter(
            (t) => t.label.toLowerCase().indexOf(value.toLowerCase()) > -1
          )
        })
      },
