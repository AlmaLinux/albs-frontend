  ...
  <div class="row no-wrap no-scroll q-pa-md">
      <table class="q-table horizontal-separator full-width col-lg-12 col-xl-10">
        ...
        <tr class="bg-grey-2">
          ...
          <template v-else>
          <template v-for="log in testsStatusFilter(testsFilter(test.result, search), statusFilter)" :key="log">
          <tr>
            ...
            <td v-if="log.tap && !search"
              ...
              <td class="text-center" width="15%" @click="onView(log, test.revision)">
                  <q-chip
          ...
          <template v-if="log.tap">
              <tr v-for="tap in tapFilter(log, search)"
                  :key="tap.test_name">
                <td colspan="3" style="padding-left: 3rem; word-break: break-all;">
                  {{ tap.test_name }}
                </td>
                <td class="text-center">
                  <q-chip
