        ...
        <template v-slot:body="props">
          <q-tr :props="props">
            ...
            <q-td key="date" :props="props">
              <span
                :class="props.row.created_at ? null : 'text-caption text-grey'"
              >
            ...
            <q-td key="packages" :props="props">
              <template
                v-for="project in getProjects(props.row.plan)"
                :key="project.name"
              >
                <router-link
                  v-if="project.buildId"
...
<script>
  ...
  export default defineComponent({
    ...
    computed: {
      ...
      currentPage: {
        get() {
          return this.$store.state.releaseFeed.pageNumber
        },
        set(value) {
          this.loading = true
    ...
    methods: {
      ...
      getProjects(plan) {
        ...
        plan.packages.forEach((pack) => {
          ...
          if (pack.package.source) {
            let rpm = splitRpmFileName(pack.package.source)
            if (!setPackages.has(rpm.name)) {
              packages.push({
                name: rpm.name,
                version: rpm.version,
                release: rpm.release,
      ...
      warningLogs(release) {
        ...
        if (release.plan.last_log) {
          if (release.plan.last_log.includes('WARNING')) warning = true
        }
        return warning
      },
    },
