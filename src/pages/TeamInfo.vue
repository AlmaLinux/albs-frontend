      ...
      <q-form @submit="addUsers">
        <q-card-section>
          <q-select
            ...
            clearable
            :rules="[
              (val) => usersRule(val) || 'Please select at least one user',
            ]"
    ...
    computed: {
      allUsers() {
        return this.$store.state.users.users.filter((user) => {
          let index = this.team.members.findIndex(
    ...
    methods: {
      userFilter(val, update, abort) {
        update(() => {
          ...
          this.usersOptions = this.allUsers.filter(
      ...
      updateMembers(data, modification) {
        this.$api
          .post(`/teams/${this.teamId}/members/${modification}/`, data)
          .then((response) => {
      ...
      deleteTeam() {
        ...
        this.$api
          .delete(`/teams/${this.teamId}/remove`)
          .then((response) => {
            this.confirm = false
      ...
      async setCanEditRoles() {
        ...
        let managerRole = this.team.roles.find(
          (role) => role.name == `${this.team.name}_manager`
        )
