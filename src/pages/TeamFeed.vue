          ...
          <q-tr :props="props">
            <q-td auto-width>
              <q-btn
                ...
                round
                icon="info"
                :to="{path: `/teams/${props.row.id}`, query: {tab: 'members'}}"
              />
            </q-td>
            ...
            <q-td key="products" :props="props">
              <router-link
                :to="{path: `/teams/${props.row.id}`, query: {tab: 'products'}}"
                :id="`tfe-li-products-${props.row.id}`"
              >
            ...
            <q-td key="members" :props="props">
              <router-link
                :to="{path: `/teams/${props.row.id}`, query: {tab: 'members'}}"
                :id="`tfe-li-members-${props.row.id}`"
              >
            ...
            <q-td auto-width>
              <q-btn
                ...
                icon="delete"
                class="del-btn"
                @click="selectDeletingTeam(props.row)"
              />
            </q-td>
          </q-tr>
        </template>
      ...
      <q-form @submit="newTeam">
        <q-card-section>
          <q-input
            v-model="newTeamName"
            label="Name*"
            hint="Enter a name for a new team"
...
<script>
  ...
  export default defineComponent({
    ...
    computed: {
      ...
      currentPage: {
        get() {
          return this.$store.state.teams.pageNumber
        },
        set(value) {
